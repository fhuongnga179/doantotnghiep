...
@section('content')
    ...
    <div class="carts">
        ...
        <a class="btn btn-danger" href="{{ route('admin.carts.refuse', ['id' => $customer->id]) }}">
            Từ chối đơn
        </a>
        <a class="btn btn-success" href="{{ route('admin.carts.approve', ['id' => $customer->id]) }}">
            <i class="fas fa-check"></i> Duyệt đơn hàng
        </a>


        {{-- <a href="{{ url('preview') }}"> In đơn hàng</a> --}}
    </div>
@endsection
